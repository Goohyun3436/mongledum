...
import { FiArrowUpRight, FiChevronDown, FiMail, FiMusic, FiSend } from "react-icons/fi";

const contactLinks = [
  { label: "youtube", account: "@mongledum_official", href: "https://www.youtube.com/@mongledum_official", icon: FaYoutube },
    ...
    <main className="contact-page">
      ...
      <section className="contact-links" aria-label="몽글덤 연락처와 소셜 미디어">
        <div className={`contact-email${isEmailOpen ? " is-open" : ""}`}>
          <button className="contact-email__trigger" type="button" aria-expanded={isEmailOpen} aria-controls="contact-email-form" onClick={() => setIsEmailOpen((current) => !current)}>
            <FiMail className="contact-links__icon" aria-hidden="true" />
            <span className="contact-links__content"><strong>email</strong><small>mongledum@gmail.com</small></span>
            <FiChevronDown className="contact-email__chevron" aria-hidden="true" />
          </button>
          <div className="contact-email__dropdown" id="contact-email-form" hidden={!isEmailOpen}>
            <form className="contact-email__form" onSubmit={sendEmail}>
              ...
              <label>
                ...
                <span className="contact-email__phone">
                  <input type="tel" name="contactFirst" inputMode="numeric" autoComplete="tel-national" pattern="[0-9]{2,3}" maxLength="3" aria-label="연락처 앞자리" required />
                  <i aria-hidden="true">-</i>
                  <input type="tel" name="contactMiddle" inputMode="numeric" pattern="[0-9]{3,4}" maxLength="4" aria-label="연락처 중간자리" required />
                  <i aria-hidden="true">-</i>
                  <input type="tel" name="contactLast" inputMode="numeric" pattern="[0-9]{4}" maxLength="4" aria-label="연락처 뒷자리" required />
                </span>
              </label>
              <label className="contact-email__subject"><span>제목</span><input type="text" name="subject" required /></label>
              <label className="contact-email__message"><span>내용</span><textarea name="message" rows="4" required /></label>
              <section className="contact-email__privacy" aria-labelledby="contact-privacy-title">
                <h3 id="contact-privacy-title">개인정보 수집 및 이용 동의 <span aria-hidden="true">*</span></h3>
                <div className="contact-email__privacy-content" tabIndex="0">
                  <p>몽글덤은 문의 접수와 답변을 위해 아래와 같이 개인정보를 수집·이용합니다.</p>
                  <dl>
                    <div><dt>수집·이용 목적</dt><dd>문의 내용 확인, 답변 및 연락</dd></div>
                    <div><dt>수집 항목</dt><dd>이름, 연락처</dd></div>
                    <div><dt>보유 및 이용 기간</dt><dd>문의 처리 완료 후 1년</dd></div>
                  </dl>
                  <p>동의를 거부할 권리가 있으나, 필수 정보 수집에 동의하지 않으면 문의 접수와 답변이 제한됩니다.</p>
                </div>
              ...
              <button className="contact-email__submit" type="submit"><FiSend aria-hidden="true" />메일 보내기</button>
            </form>
          </div>
        </div>
